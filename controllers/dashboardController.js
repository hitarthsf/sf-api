...
export const getData = async (req, res) => {
  ...
  try {
    ...
    if (location_id[0] === "") {
      ...
      if (fetchedLocations) {
        if (
          fetchedLocations.location !== undefined &&
      ...
      // Employee id filter
      if (employee_id) {
        var employee = await RatingEmployeeData.find({
          employee_id: employee_id,
        });
        var employee_rating_id = [];
        employee_rating_id = await employee.map((employeeData) =>
          employeeData.rating_id.toString()
        );
    ...
    if (detractor.length != 0 && promoters.length != 0) {
      const nps =
        promoters[0]["rating"] / count_total[0]["rating"] -
        detractor[0]["rating"] / count_total[0]["rating"];
    }

    const ratings = [
      {
        count: count_total[0]["rating"],
...
export const getLocationRank = async (req, res) => {
  ...
  try {
    ...
    if (location_rank.length === 0) {
      res
        .status(200)
        .json({ data: location_rank, message: "No data on this filter" });
    }
    // get location name
    ...
    if (!companyData) {
      res
        .status(200)
        .json({ data: location_rank, message: "Invalid Company Id " });
    }
...
export const getRatingsDistribution = async (req, res) => {
  ...
  try {
    ...
    if (average_count.length === 0) {
      const ratings = [
        { distribution: distribution, countDistribution: countDistribution },
      ];
      res
        .status(200)
        .json({ data: ratings, message: "No data found with above filter" });
    }
  ...
  try {
    ...
    if (format == "chart") {
      ...
      if (!companyData) {
        res
          .status(200)
          .json({ data: skillRanks, message: "Invalid Company Id " });
      }
        ...
        companyData.attributes.forEach((attribute) => {
          ...
          if (count > 0) {
            if (AttributeName.includes(attribute.name)) {
              var index = AttributeName.findIndex(
      ...
      var data = [
        { AttributeName: AttributeName, AttributeCount: AttributeCount },
      ];
  ...
  try {
    ...
    if (format == "chart") {
      ...
      if (type == "positive" && SkillNamePositive.length > 0) {
        var data = [
          { SkillName: SkillNamePositive, SkillCount: SkillCountPositive },
        ];
      }
      if (type == "negative" && SkillNameNegative.length > 0) {
        var data = [
          { SkillName: SkillNameNegative, SkillCount: SkillCountNegative },
        ];
      }
...
export const getUserStats = async (req, res) => {
  try {
    ...
    const lastWeekLoggedInUserList = await UserLoginData.aggregate([
      {
        $match: {
          createdAt: { $gte: sevenDaysDateFrom, $lte: sevenDaysDateTo },
        },
      },
      { $group: { _id: "$user_id", count: { $sum: 1 } } },
    ]);
    const totalLastWeekLoggedInUser = _.sumBy(
      lastWeekLoggedInUserList,
      function (data) {
        return data.count;
      }
    );
    ...
    const totalRiskLoggedInUser = _.sumBy(
      riskLoggedInUserList,
      function (data) {
        return data.count;
      }
    );
    ...
    const responseData = {
      ...
      disEngagedUserCount:
        totalUser - totalLastWeekLoggedInUser - totalRiskLoggedInUser,
    };
    res.status(200).json({ data: responseData, message: "Success" });
  } catch (error) {
    res.status(404).json({ message: error.message });
...
export const getUserStatDetails = async (req, res) => {
  ...
  try {
    if (type === "activeUsers") {
      const sevenDaysDateTo = new Date(moment().toISOString());
      const sevenDaysDateFrom = new Date(
        moment().subtract(21, "d").toISOString()
      );
      const lastWeekLoggedInUserList = await UserLoginData.aggregate([
        {
          $match: {
      ...
      res
        .status(200)
        .json({ data: lastWeekLoggedInUserList, message: "Success" });
      ...
      const riskDaysDateTo = new Date(moment().subtract(8, "d").toISOString());
      const riskDaysDateFrom = new Date(
        moment().subtract(21, "d").toISOString()
      );
      const sevenDaysLoggedInUserArray = _.map(lastWeekLoggedInUserList, "_id");
      ...
      const engagedUserNotInarray = [
        ...sevenDaysLoggedInUserArray,
        ...riskLoggedInUserArray,
      ];
