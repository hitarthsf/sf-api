...
export const addScreenSaver = async(req,res) => {
    ...
    if ( !title || !type ) {
        res.status(409).json({ data: [], message : 'Invalid request, one or multiple fields are missing.'});
    }
   ...
   try {
        ...
        res.status(201).json({data: screenSaver, message: "Screen Saver Created Successfully !!"});
   } catch (error) {
...
export const editScreenSaver = async(req,res) => {
   ...
   try {
        ...
        res.status(201).json({data: screenSaverObj, message: "Screen Saver Updated Successfully !!"});
   } catch (error) {
       res.status(409).json({ message : error.message})
   }
}
